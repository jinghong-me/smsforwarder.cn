...
轻量、开源的 Android 应用，兼具短信转发与完整 SMS 客户端功能。支持设置为 Android 14/15 (HyperOS) 的默认短信应用，也可在非默认模式下独立运行转发功能。
...
## 默认短信应用模式（Android 10+ / HyperOS）

### 功能说明
从 v3.0 起，本应用支持设置为系统**默认短信应用**，具备以下能力：
- 接收 `SMS_DELIVER` 广播（高优先级，不会被其他应用拦截）
- 自动将收到的短信写入系统收件箱（`content://sms/inbox`）
- 在"短信"标签页中查看会话列表和收发短信
- 在非默认模式下仍可接收 `SMS_RECEIVED` 广播并完成转发（功能不变）

### 设置为默认短信应用
1. 打开应用，切换到"配置"标签页
2. 点击"设置为默认短信应用"按钮（或通过"短信"标签页顶部的横幅提示）
3. 在系统弹出的对话框中确认即可

#### HyperOS / MIUI 特别提示
- 进入 **设置 → 应用 → 默认应用 → 短信应用**，选择"短信转发助手"
- 需授予的权限：接收短信、发送短信、读取短信、写入短信、读取联系人（可选）、通知

### 所需权限说明
| 权限 | 用途 | 模式 |
|------|------|------|
| RECEIVE_SMS | 接收短信广播（非默认模式） | 通用 |
| SMS_DELIVER | 接收短信广播（默认应用模式） | 默认模式 |
| SEND_SMS | 发送短信（转发通道 & 短信客户端） | 通用 |
| READ_SMS | 读取系统短信数据库 | 默认模式 |
| WRITE_SMS | 写入系统短信数据库 | 默认模式 |
| RECEIVE_MMS | 接收彩信广播（存根，满足角色要求） | 默认模式 |
| INTERNET | 发送 Webhook 转发 | 通用 |
| POST_NOTIFICATIONS | 显示来信通知 | 通用 |

### 安全设计
- **Webhook URL 强制 HTTPS**：非 HTTPS 地址会被跳过并记录日志
- **日志中手机号脱敏**：仅保留末 4 位（例：`****3456`）
- **网络操作不在 BroadcastReceiver 中进行**：收到短信后通过 WorkManager 异步转发，避免 ANR
- **allowBackup=false**：防止备份泄露配置与日志数据
- **幂等转发**：相同 messageId 不重复转发（避免开机广播触发重复操作）
...
- 需要在 Android 手机上同时作为完整短信客户端使用的场景。
